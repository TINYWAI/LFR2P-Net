...
def bda_collate(batch):
    ...
    for sample in batch:
        imgs_A.append(sample[0])
        imgs_B.append(sample[1])
        label1s.append(sample[2])
        label2s.append(sample[3])

    return torch.stack(imgs_A, 0), torch.stack(imgs_B, 0), torch.stack(label1s, 0), torch.stack(label2s, 0)
