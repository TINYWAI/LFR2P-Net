...
def compute_disaster_type_metrics(disaster_type_evaluator_dict):
    """Computes and prints mIoU for each disaster type."""
    print("\nPer-Disaster Type mIoU:")
    average_mIoU = 0
    disaster_list, miou_list, iou_list = [], [], []
    for disaster_type, evaluator in disaster_type_evaluator_dict.items():
        mean_iou = evaluator.Mean_Intersection_over_Union()
        iou_per_class = evaluator.Intersection_over_Union()
        average_mIoU += mean_iou

        disaster_list.append(disaster_type)
        miou_list.append(mean_iou)
        iou_list.append(iou_per_class)

    average_mIoU = average_mIoU / len(disaster_list)

    return disaster_list, miou_list, iou_list, average_mIoU


def compute_disaster_event_metrics(disaster_event_evaluator_dict):
    """Computes and prints mIoU for each disaster event."""
    print("\nPer-Event Type mIoU:")
    average_mIoU = 0
    event_list, miou_list, iou_list = [], [], []
    for event, evaluator in disaster_event_evaluator_dict.items():
        mean_iou = evaluator.Mean_Intersection_over_Union()
        iou_per_class = evaluator.Intersection_over_Union()
        average_mIoU += mean_iou
    ...
    average_mIoU = average_mIoU / len(event_list)

    return event_list, miou_list, iou_list, average_mIoU
