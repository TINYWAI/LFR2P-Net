...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, feat_dict, logits, labels):
...
class CrossEntropyLoss2d(nn.Module):
    ...
    def forward(self, feat_dict, logits, labels):
        ...
        for outputs in logits:
            ...
            labels_ = labels.clone().unsqueeze(1).float()
            labels_ = F.interpolate(labels_, size=(H, W), mode='nearest')
            labels_ = labels_.squeeze(1).long()
            loss = loss + self.ce_loss(outputs, labels_)
            if self.use_dice:
                loss += self.dice_loss(outputs, labels_)
