...
class SemanticDecoder(nn.Module):
    def __init__(self, encoder_dims, proj_dim, channel_first, norm_layer, ssm_act_layer, mlp_act_layer, **kwargs):
        super(SemanticDecoder, self).__init__()

        # Define the VSS Block for Spatio-temporal relationship modelling
        self.st_block_4_semantic = nn.Sequential(
            nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-1], out_channels=proj_dim),
        ...
        self.trans_layer_3 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-2], out_channels=proj_dim),
                                          nn.BatchNorm2d(proj_dim), nn.ReLU())
        self.trans_layer_2 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-3], out_channels=proj_dim),
                                          nn.BatchNorm2d(proj_dim), nn.ReLU())
        self.trans_layer_1 = nn.Sequential(nn.Conv2d(kernel_size=1, in_channels=encoder_dims[-4], out_channels=proj_dim),
                                          nn.BatchNorm2d(proj_dim), nn.ReLU())


        # Smooth layer
        self.smooth_layer_3_semantic = ResBlock(in_channels=proj_dim, out_channels=proj_dim, stride=1)
        self.smooth_layer_2_semantic = ResBlock(in_channels=proj_dim, out_channels=proj_dim, stride=1)
        self.smooth_layer_1_semantic = ResBlock(in_channels=proj_dim, out_channels=proj_dim, stride=1)
        self.smooth_layer_0_semantic = ResBlock(in_channels=proj_dim, out_channels=proj_dim, stride=1)
